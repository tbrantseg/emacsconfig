...
** ROOT mode
There's not much in the way of ROOT support on Emacs - it's a shame, but then again ROOT has quite a lot of issues, so it's not too surprising. There is at least one package included with ROOT, so we may as well make sure we load that.

#+BEGIN_SRC emacs-lisp
  (add-to-list 'load-path "/opt/local/libexec/root6/share/emacs/site-lisp/")
  (require 'root-help)
#+END_SRC
